          ...
          let rec delete list =
            ...
            | head :: tail -> begin
                if file_exists head && not (is_directory head) then begin
                  (* Invalidate cached buffer before the file goes away,
                     otherwise a later load would hit stale content. *)
                  Bcs_buffer_cache.invalidate_path head ;
                  if do_backup then
          ...
          let move src dst =
            ...
            if not (is_directory src) then
              if (file_exists dst) then begin
                ...
                (* The src resref loses its backing file; dst may be
                   replacing an existing cached resref. Invalidate both. *)
                Bcs_buffer_cache.invalidate_path src ;
                Bcs_buffer_cache.invalidate_path dst ;
          ...
          if !find_list <> [] then begin
            ...
            Hashtbl.iter (fun name biff ->
              ...
              with e ->
                ...
                raise e)) game.Load.loaded_biffs;
            game.Load.loaded_biffs <- Hashtbl.create 5 ;
            (* The KEY was just reloaded — any BCS/BAF buffers cached
               from the pre-biff state now have paths pointing at the
               old override files. Contents are still correct, but a
               full clear is the safe, cheap invalidation. *)
            Bcs_buffer_cache.clear () ;
                  ...
                  let result_buff =
                    List.fold_left (fun acc elt ->
                      ...
                      | Invalid_argument msg ->
                          log_and_print
                            "WARNING: [%s] -> [%s] Patching skipped due to Invalid_argument (%s). Returning file unchanged.\n"
                            src dest msg;
                          acc
                      | e -> log_and_print
      ...
      | TP_DecompressBiff(sl) ->
          ...
          Hashtbl.iter (fun name biff ->
            ...
            with e ->
              ...
              raise e)) game.Load.loaded_biffs;
          game.Load.loaded_biffs <- Hashtbl.create 5 ;
          (* Biffs are about to be relocated/decompressed. Clear cached
             BCS/BAF buffers so subsequent reads re-resolve through the
             post-decompress layout. *)
          Bcs_buffer_cache.clear () ;
