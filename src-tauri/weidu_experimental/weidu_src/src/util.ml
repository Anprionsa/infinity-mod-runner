...
let open_for_writing_internal backup filename binary =
  (* Invalidate the BCS/BAF buffer cache for this resource before the
     write lands. No-op for non-cacheable extensions. See
     bcs_buffer_cache.ml for the invalidation contract. *)
  Bcs_buffer_cache.invalidate_path filename ;
