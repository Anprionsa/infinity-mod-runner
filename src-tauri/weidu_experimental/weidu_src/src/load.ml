...
let load_resource for_what game override_allowed name ext =
  (* Consume the skip flag up front so a cache hit still resets it — the
     flag is scoped to "the next load call", not specifically "the next
     load that hit disk". *)
  let skip_this_error = !skip_next_load_error in
  skip_next_load_error := false ;
  (* BCS/BAF buffer cache short-circuit. No-op for other extensions.
     See bcs_buffer_cache.ml for scope + invalidation contract. *)
  match Bcs_buffer_cache.lookup name ext with
  | Some cached -> cached
  | None ->
  ...
  let final =
    if ((ext_up = "IDS" || ext_up = "2DA") && Xor.is_encrypted a) then
      ...
      (a,b)
  in
  (* Populate cache. insert() is a no-op for non-BCS/BAF extensions and
     for empty buffers (the "(resource not found)" sentinel case). *)
  let (buf, path) = final in
  Bcs_buffer_cache.insert name ext buf path ;
  final
