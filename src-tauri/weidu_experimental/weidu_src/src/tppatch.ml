...
(* Flag to indicate we are inside a DECOMPILE_AND_PATCH block.
   When true, BAF->BCS recompilation failures are non-fatal:
   the original BCS is returned unchanged with a warning.
   This prevents round-trip failures (caused by IDS state changes
   in large mod stacks) from aborting entire mod installations. *)
let decompile_and_patch_context = ref false
...
(*           ( [] , [] ) *) in
    ...
    | TP_CompileBAFtoBCS ->
        ...
        with e ->
          Dc.ok_to_resolve_strings_while_loading := old_ok;
          if !decompile_and_patch_context then begin
            log_and_print "WARNING: [%s] cannot be decompiled and recompiled (round-trip failure). Returning original BCS unchanged.\n"
              patch_filename;
            buff
          end else if List.mem (String.uppercase patch_filename)
              ["RDOG.BCS"; "RDWARF.BCS";
    ...
    | TP_CompileBCStoBAF ->
        ...
        with e ->
          if !decompile_and_patch_context then begin
            log_and_print "WARNING: [%s] cannot be decompiled (BCS to BAF failure). Returning original buffer unchanged.\n"
              patch_filename;
            buff
          end else if List.mem (String.uppercase patch_filename)
              ["RDOG.BCS"; "RDWARF.BCS";
    ...
    | TP_DecompileAndPatch pl ->
        ...
        let old_context = !decompile_and_patch_context in
        decompile_and_patch_context := true;
        let ans =
          process_patch2 patch_filename game
            (List.fold_left
               ...
               pl)
            com in
        decompile_and_patch_context := old_context;
        ans
